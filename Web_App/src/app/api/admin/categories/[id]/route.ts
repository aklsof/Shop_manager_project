...
export async function PUT(
  ...
  try {
    ...
    // Rename the category in the product_categories table
    // Because products now use category_id, we DON'T need to update the products table!
...
export async function DELETE(
  ...
  try {
    // Safety check: refuse deletion if products still use this category_id
    ...
    if (rows[0].cnt > 0) {
      return NextResponse.json(
        { error: `Cannot delete: ${rows[0].cnt} product(s) are still assigned to this category. Please reassign them first.` },
        { status: 409 }
      );
    }
