...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const [result] = await pool.query<ResultSetHeader>(
      'INSERT INTO price_rules (product_id, rule_type, promotional_price, start_date, end_date, is_active) VALUES (?, ?, ?, ?, ?, ?)',
      [product_id, rule_type, promotional_price, start_date, end_date, is_active ?? 1]
    );
