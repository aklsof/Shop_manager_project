...
-- PHP Version: 8.2.12
--
-- Corrected version (v4) - Mar 29, 2026
-- Fix 1: Added ENGINE=InnoDB to `price_rules` table (was missing).
-- Fix 2: Removed DEFINER=`root`@`localhost` from all 8 views to allow
--        import by any sufficiently privileged user, not just root.
-- Fix 3: Changed `users`.`role` from NOT NULL to DEFAULT NULL so that
--        client accounts are not forced to carry a staff role value.
-- Fix 4: Added FOR UPDATE lock in `trg_sale_deduct_stock` trigger to
--        prevent a race condition on concurrent stock deductions.
...
--
DELIMITER $$
CREATE  TRIGGER IF NOT EXISTS `trg_adjustment_apply` AFTER INSERT ON `inventory_adjustments` FOR EACH ROW BEGIN
...
--
DELIMITER $$
CREATE  TRIGGER IF NOT EXISTS `trg_validate_price_rule` BEFORE INSERT ON `price_rules` FOR EACH ROW BEGIN
...
(1, 'Standard', 19.00, '2026-03-16 19:29:48') ON DUPLICATE KEY UPDATE 
    `rate` = VALUES(`rate`),
...
--
DELIMITER $$
CREATE  TRIGGER IF NOT EXISTS `trg_default_receipt_language` BEFORE INSERT ON `transactions` FOR EACH ROW BEGIN
...
DELIMITER ;
DELIMITER $$
CREATE  TRIGGER IF NOT EXISTS `trg_prevent_inactive_user_login` BEFORE INSERT ON `transactions` FOR EACH ROW BEGIN
...
DELIMITER ;
DELIMITER $$
CREATE  TRIGGER IF NOT EXISTS `trg_transaction_total_verify` BEFORE INSERT ON `transactions` FOR EACH ROW BEGIN
...
--
DELIMITER $$
CREATE  TRIGGER IF NOT EXISTS `trg_refund_create_lot` AFTER INSERT ON `transaction_items` FOR EACH ROW BEGIN
...
DELIMITER ;
DELIMITER $$
CREATE  TRIGGER IF NOT EXISTS `trg_sale_deduct_stock` AFTER INSERT ON `transaction_items` FOR EACH ROW BEGIN
    -- Only deduct for sales (positive quantity); refunds are handled by T2
    IF NEW.quantity > 0 THEN
        -- Guard: lock the row and ensure sufficient stock exists in this lot
...
CREATE TABLE IF NOT EXISTS `users` (
  ...
  `role` enum('Store Associate','Administrator') DEFAULT 'Store Associate',
...
--
DROP VIEW IF EXISTS `vw_active_price`;
CREATE TABLE IF NOT EXISTS `vw_active_price` (
`product_id` int(11)
...
-- (See below for the actual view)
--
DROP VIEW IF EXISTS `vw_associate_sales_summary`;
CREATE TABLE IF NOT EXISTS `vw_associate_sales_summary` (
`user_id` int(11)
,`username` varchar(50)
...
-- (See below for the actual view)
--
DROP VIEW IF EXISTS `vw_fifo_lot_queue`;
CREATE TABLE IF NOT EXISTS `vw_fifo_lot_queue` (
`product_id` int(11)
,`product_name` varchar(100)
,`lot_id` int(11)
...
-- (See below for the actual view)
--
DROP VIEW IF EXISTS `vw_financial_report`;
CREATE TABLE IF NOT EXISTS `vw_financial_report` (
`sale_date` date
,`product_id` int(11)
,`product_name` varchar(100)
,`category` varchar(50)
...
-- (See below for the actual view)
--
DROP VIEW IF EXISTS `vw_inventory_adjustment_log`;
CREATE TABLE IF NOT EXISTS `vw_inventory_adjustment_log` (
...
CREATE TABLE IF NOT EXISTS `vw_low_stock_alerts` (
`product_id` int(11)
,`product_name` varchar(100)
,`category` varchar(50)
...
CREATE TABLE IF NOT EXISTS `vw_product_stock` (
`product_id` int(11)
,`product_name` varchar(100)
,`category` varchar(50)
,`store_location` varchar(100)
...
-- (See below for the actual view)
--
DROP VIEW IF EXISTS `vw_web_orders_dashboard`;
CREATE TABLE IF NOT EXISTS `vw_web_orders_dashboard` (
`order_id` int(11)
,`status` enum('Pending','Ready for Pickup','Completed')
,`order_date` timestamp
,`updated_at` timestamp
,`client_id` int(11)
,`client_username` varchar(50)
,`client_preferred_lang` varchar(10)
,`handled_by_user` varchar(50)
,`product_id` int(11)
,`product_name` varchar(100)
,`store_location` varchar(100)
,`quantity` int(11)
...
--
DELIMITER $$
CREATE  TRIGGER IF NOT EXISTS `trg_order_status_workflow` BEFORE UPDATE ON `web_orders` FOR EACH ROW BEGIN
